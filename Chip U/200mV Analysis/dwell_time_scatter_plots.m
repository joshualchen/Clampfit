...
for x = 1:length(files)
    ...
    if endsWith(name, "events_complete.CSV") || endsWith(name, "events_complete.csv")
        ...
        for i = 1:height(structure)
            if not(ismissing(structure{i, 5}))
                base = structure{i, 11};
...
%                 end
                ...
                pos9 = (pos5(1) - pos3(1))/1000;  % difference in dwell times
                pos10 = pos4(2);
                if structure{i, 4} == 'T'
...
figure(2) 
W_dwell_times = cell2mat(W_data(:, 9));
[counts,edges] = histcounts(log10(W_dwell_times));
histogram(W_dwell_times, 10.^edges)
title("W Events Histogram -- 3-5 Dwell Time")
xlabel("Dwell Time (ms)")
set(gca,'xscale','log')
...
figure(3) 
T_dwell_times = cell2mat(T_data(:, 9));
[counts,edges] = histcounts(log10(T_dwell_times));
...
title("T Events Histogram -- 3-5 Dwell Time")
xlabel("Dwell Time (ms)")
